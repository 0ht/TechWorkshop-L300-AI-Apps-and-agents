...
class AgentFrameworkProductManagementAgent:
    ...
    def _get_agent_response(
        ...
        If the model did not return JSON matching ``ResponseFormat`` and instead
        returned plain text, fall back to treating the whole message as the
        ...
        # Try to parse the message as JSON matching ResponseFormat. When the
        # model returns normal text (e.g. "Short answer: ..."), this will fail
        # with a ValidationError; in that case we just surface the text.
        try:
            structured_response = ResponseFormat.model_validate_json(message)
        except ValidationError as exc:
            logger.warning(
                "Failed to parse structured response as JSON; falling back to plain text. Error=%s, message=%r",
                exc,
                message,
            )
            return {
